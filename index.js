...
app.get('/', async (req, res) => {
    ...
    try {
        ...
        res.render('homepage', { title: 'Freezer Items | Integrating With HubSpot I Practicum', data });
    } catch (error) {
        console.error(error);
    }
});
...
app.post('/update-cobj', async (req, res) => {
    const newItem = {
        properties: {
            ...
            date_frozen: req.body.date_frozen,
            location_description: req.body.location_description
        }
    };

    const customObjectUrl = 'https://api.hubspot.com/crm/v3/objects/2-64863094';
    const headers = {
        Authorization: `Bearer ${PRIVATE_APP_ACCESS}`,
        'Content-Type': 'application/json'
    };
    ...
    try {
        await axios.post(customObjectUrl, newItem, { headers });
        res.redirect('/');
    } catch (error) {
        console.error(error);
    }
